...
\begin{algorithm2e}[H]
	...
	\uIf{$\rho$ is a concatenation}
	{
		...
		\While{$i \geq 2$}
		{
			\uIf{$\alpha_{i}$ is an option \KwAnd $\child(\alpha_{i})$ is a plus}
			{
				...
				{
					$\gamma \leftarrow \conc(\alpha_{j},\ldots,\alpha_{i-1})$\;
					\uIf{$L(\gamma)=L(\beta)$}
					{
						replace position $i$ in $c$ with $\beta\rxp$\tcp*[r]{replace $\alpha_i$ with $\beta\rxp$} 
						replace positions $j$ to $(i-1)$ in $c$ with $\emptyword$\tcp*[r]{consider those deleted} 
...
\begin{algorithm2e}[H]
	...
	\uIf{$\rho$ is a concatenation}
	{
		...
		{
			$\changed\leftarrow\false$\;
			\If{$\alpha_{i}$ is an option \KwAnd $\child(\alpha_{i})$ is a plus}
			{
				$\beta\leftarrow \child(\alpha_{i})$ \tcp*[r]{Hence, $\alpha_i=\beta\rxp\rxo$}
				\If{$\alpha_{i-1}$ is an option \KwAnd $\child(\alpha_{i-1})$ is a concatenation}
				{
...
TODO: Explain rule~P4. TODO: Example.

...
\begin{algorithm2e}[H]
	...
	\uIf{$\rho$ is a concatenation}
	{
		...
		\While{$i \geq 2$}
		{
			...
			{
				$\alpha\leftarrow \child(\rho_{i})$ \tcp*[r]{Hence, $\rho_i=\alpha\rxp\rxo$}
				$j\leftarrow (i-1)$\;
...
\begin{algorithm2e}[H]
	...
	\uIf{$\rho$ is a concatenation}
	{
		...
		\While{$i \geq 2$}
		{
			...
			{
				$\alpha\leftarrow \child(\rho_{i})$ \tcp*[r]{Hence, $\rho_{i}=\alpha\rxp\rxo$}
...
\begin{align*}
	...
	\projfirst_{A}((\beta_1\rxc\cdots\rxc\beta_n))&\df
		...
		\end{cases}
\end{align*}
Note that we assume $\emptyset$ and $\emptyword$ to be eliminated as explained for the  extended syntax of (D)REs (with $\emptyset$ and $\emptyword$), see Section~\ref{sec:dredef}. Furthermore, if $A\supseteq\term(\alpha)$, $\projfirst_{A}(\alpha)$ holds by definition.

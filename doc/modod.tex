...
The auxiliary function $\pnfuphfun$ is defined by
\begin{align*}
...
\begin{algorithm2e}[H]
	...
	\If{$\rho$ is a choice}
	{
		...
		\lIf{a child of $\rho$ was rewritten}{ensure the whole DRE is in PNF}
		unmark all marked nodes\;
	}
	\ForEach{$\gamma\in\children(\rho)$}
	{
		\lIf{$\gamma$ is not a terminal}{$\factorOut(\gamma)$}
	}
	\caption{\texttt{factorOut}}
\end{algorithm2e}

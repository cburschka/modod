...
\begin{algorithm2e}[H]
	...
	\If{$\rho$ is a choice}
	{
		...
		\If{a child of $\rho$ was rewritten}
		{
			\If{$(\rn\;\KwAnd\;\KwNot(\nullfun(\rho)\;\KwOr\;\nullfun(\parent(\rho))))$}
			{
				add an option node above $\rho$\; % insert new ? if needed
			}
			ensure the whole DRE is in PNF and well-formed\;
		}
	...
	\caption{\texttt{factorOut(}$\rho$\texttt{)}}
\end{algorithm2e}

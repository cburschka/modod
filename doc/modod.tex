...
\begin{align*}
	...
	(\emptyword \ror \alpha)\to\alpha\rxo,\ (\alpha \ror \emptyword)&\to \alpha\rxo, & (\emptyset \ror \alpha)\to\alpha,\ (\alpha \ror \emptyset) &\to \alpha,\\
...
Warning: This algorithm does not preserve PNF.
...
Let $\Sigma$ be an alphabet, and let $A\subseteq \Sigma$. We define the function $\projfirst_A$ (\emph{\textbf{p}roject \textbf{f}irst}) from and to DREs over $\Sigma$ as follows:
\begin{align*}
	\projfirst_A(a)&\df 
		\begin{cases}
			a & \text{ if $a\in A$,}\\
	...
	\projfirst_{A}((\beta_1\rxc\cdots\rxc\beta_n))&\df
		\begin{cases}
			...
			\projfirst_{A}(\rlo(\beta_1))\rxc\conc(\beta_2\ldots\beta_n) & \text{ if $\emptyword\in L(\beta_1)$, $\projfirst_{A}(\conc(\beta_2\ldots\beta_n))=\emptyset$}\\
			\projfirst_{A}(\beta_1)\rxc\projfirst_{A}(\conc(\beta_2\ldots\beta_n)) & \text{ if $\emptyword\in L(\beta_1)$, $\projfirst_{A}(\conc(\beta_2\ldots\beta_n))\neq\emptyset$},
		\end{cases}
\end{align*}
where the auxiliary function $\rlo$ (\emph{\textbf{r}emove \textbf{l}eftmost \textbf{o}ption}) is defined by
\begin{align*}
	\rlo(a)&\df a \text{ for all $a\in\Sigma$},\\
	\rlo(\alpha\rxo)&\df \alpha,\\
	\rlo(\alpha\rxp)&\df \alpha\rxp,\\
	\rlo((\beta_1\ror\cdots\ror\beta_n))&\df (\rlo(\beta_1)\ror\cdots\ror\rlo(\beta_n)),\\
	\rlo((\beta_1\rxc\beta_2\rxc\cdots\rxc\beta_n))&\df (\rlo(\beta_1)\rxc\beta_2\rxc\cdots\rxc\beta_n).
\end{align*}
...
Note to Dominik: See 12.170f for further examples. (TODO: Include them here.)

Let $\alpha\df ((a\rxo \rxc b) \ror c )$. Then
\begin{align*}
	\projfirst_{\{a\}}(\alpha)&=(a\rxc b), &
	\projfirst_{\{b\}}(\alpha)&=b, &
	\projfirst_{\{c\}}(\alpha)&=c,\\
	\projfirst_{\{a,b\}}(\alpha)&=(a\rxo \rxc b), &
	\projfirst_{\{a,c\}}(\alpha)&=((a\rxc b)\ror c), &
	\projfirst_{\{b,c\}}(\alpha)&=(b\ror c),\\
...
\begin{algorithm2e}[H]
	...
	\Else(\tcc*[f]{$\rho$ is a concatenation})
	{
		...
		{
			$\alpha\leftarrow \child(\child(\rho_{n}))$\;
			...
			{
				$\overline{A}\leftarrow (\termfun(\rho)\setminus \firstfun(\beta_A))$\; % notwendig wegen rlo in pf 
				$\beta_{\overline{A}}\leftarrow \projfirst_{\overline{A}}(\beta)$\;
				\Return $(\mathtt{ruleP4}(\beta_{\overline{A}},\true)\rxo \rxc \mathtt{ruleP4}(\alpha\rxp,\true)\rxo)$\;
			}
		}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Simplification Loop}\label{sec:simpl}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{algorithm2e}[H]
	...
	$\rho_{new}\leftarrow \pnf(\rho)$\;

	\Repeat{$\rho_{new}=\rho_{old}$}
	{	
		$\rho_{old}\leftarrow \rho_{new}$\;
		
		\Repeat{$\rho_{fold}=\rho_{new}$}
		{
			$\rho_{fold}\leftarrow \rho_{new}$\;
			$\rho_{new}\leftarrow \pnf(\factorOut(\rho_{new},\false))$\;
		}
		$\rho_{new}\leftarrow \pnf(\mathtt{rewritePlus(\rho_{new})})$\;
	}
	\Return $\rho_{new}$\;
	\caption{\texttt{simplify(}$\rho$\texttt{)}}
\end{algorithm2e}
...
\begin{itemize}
	...
	\item \texttt{simplify()} Returns the result of calling $\mathtt{simplify}(\rho)$ (cf.\ Section~\ref{sec:simpl}), where $\rho$ is the current DRE.
\end{itemize}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

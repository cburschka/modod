...
\usepackage[top=5cm, bottom=3.5cm, left=2cm, right=2cm]{geometry}
\usepackage{hyperref}
...
\begin{algorithm2e}[H]
	...
	%\SetKwFunction{nullify}{nullify}

	\SetKwFunction{factRw}{factRw}
	\SetKwFunction{factRwP}{factRwPlus}

	%\Input{A DRE $\rho$ of type choice, a Boolean variable $\parnull$}
	%\SideEffects{$\rho$ is rewritten to an equivalent DRE that has no children with a common suffix}
	%\Output{true if the DRE was rewritten; false otherwise}
	%\Assert{$\rho$ is in PNF; \parnull is true iff the parent of $\rho$ is an option}
	\BlankLine
	$\rn\leftarrow (\nullfun(\rho)\;\KwOr\;\nullfun(\parent(\rho)))$\;
	mark all children of $\rho$ that are not a concatenation\;
	\While{there are unmarked children of $\rho$}
	{
		pick some child $\gamma$ of $\rho$ that is a concatenation\;
		...
		{	
			\ForEach{$v\in(\children(\rho)\setminus\{\gamma\})$}
			{
				\lIf{$L(\beta)\eqModEw L(p_\gamma)$}
				{add $\beta$ to \delList}
				\lElseIf{$(\beta$ is a concatenation \KwAnd $L(\rmc(\beta))\eqModEw L(p_\gamma))$  }
				{add $\beta$ to \redList}
			}
			\uIf{$(\redList\cup\delList)\neq \emptyset$}
			{
				$n_S\leftarrow$ number of nullable elements in $\redList\cup\{\gamma\}$\;
				$n_P\leftarrow$ number of non-nullable elements in $\redList\cup\{\gamma\}$\;
		...
		\Else
		{
			...
			\lElseIf{$L(\unify(F))=L(p_\gamma)$}{$\delList\leftarrow F$}
			\ForEach{$v\in(\children(\rho)\setminus(\{\gamma\}\cup\delList))$}
			{
				\lIf{$(\beta$ is a concatenation \KwAnd $L(\rmc(\beta))=L(p_\gamma))$}
				{add $\beta$ to \redList}
			}
			\leIf{$(\redList\cup\delList)\neq \emptyset$}
			{$\factRw(\gamma,\redList,\delList)$}
			{mark $\gamma$}
		}
	}
...
\end{algorithm2e}
The algorithm \texttt{factorOut} is called on a given DRE, with a sub-DRE~$\rho$ of type choice as parameter.  
%\SideEffects{$\rho$ is rewritten to an equivalent DRE that has no children with a common suffix}
%\Output{true if the DRE was rewritten; false otherwise}
%\Assert{$\rho$ is in PNF; \parnull is true iff the parent of $\rho$ is an option}
...
\begin{itemize}
	...
	%\item $\nullify(\alpha)$: Takes a DRE $\alpha$. If $\alpha$ is nullable, the function returns $\alpha$; if $\alpha$ is not nullable, it returns $\alpha\rxo$.
...
\begin{align*}
	...
	\end{cases}\\
	\shortintertext{with}
	\gamma_2&\df \unify(R_S\cup\{\gamma_3\})
	\shortintertext{where the DRE~$\gamma_3$ is defined by }
